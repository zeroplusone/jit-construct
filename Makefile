BIN = interpreter \
      ...
      jit-x64 jit-arm jit-x64-contraction jit-x64-clearLoops jit-x64-copyLoops jit-x64-opt
...
jit-x64-contraction: dynasm-driver.c jit-x64-contraction.h
	$(CC) $(CFLAGS) -o $@ -DJIT=\"jit-x64-contraction.h\" \
		dynasm-driver.c
jit-x64-contraction.h: jit-x64-contraction.dasc
	        $(LUA) dynasm/dynasm.lua -o $@ jit-x64-contraction.dasc
...
jit-x64-copyLoops: dynasm-driver.c jit-x64-copyLoops.h
	$(CC) $(CFLAGS) -o $@ -DJIT=\"jit-x64-copyLoops.h\" \
		dynasm-driver.c
jit-x64-copyLoops.h: jit-x64-copyLoops.dasc
	        $(LUA) dynasm/dynasm.lua -o $@ jit-x64-copyLoops.dasc
run-jit-x64-copyLoops: jit-x64-copyLoops
	./jit-x64-copyLoops progs/hello.b && objdump -D -b binary \
		-mi386 -Mx86-64 /tmp/jitcode
...
bench-jit-x64-contraction: jit-x64-contraction
	@echo
	@echo Executing Brainf*ck benchmark suite. Be patient.
	@echo
	@env PATH='.:${PATH}' BF_RUN='$<' tests/bench.py
...
bench-jit-x64-copyLoops: jit-x64-copyLoops
	@echo
	@echo Executing Brainf*ck benchmark suite. Be patient.
	@echo
	@env PATH='.:${PATH}' BF_RUN='$<' tests/bench.py
...
clean:
	$(RM) $(BIN) \
	      ...
	      jit-x64.h jit-arm.h jit-x64-contraction.h jit-x64-clearLoops.h jit-x64-copyLoops.h jit-x64-opt.h
